...
/**
 ...
 } else if (curr.getData().compareTo(data) > 1) {
 curr.setLeft(addHelper(data, curr.getLeft()));
 } else if (curr.getData().compareTo(data) < 1) {
 curr.setRight(addHelper(data, curr.getRight()));
 }
 return curr;
    ...
    private void update(AVLNode<T> curr) {
        ...
        if (left > right) {
            curr.setHeight(left + 1);
        } else if (right > left) {
            curr.setHeight(right + 1);
        } else {
            curr.setHeight(left + 1);
        }
    ...
    private AVLNode<T> rotateLeft(AVLNode<T> node) {
        System.out.println("Rotating Left with " + node);
    ...
    private AVLNode<T> rotateRight(AVLNode<T> node) {
        System.out.println("Rotating Right with " + node);
