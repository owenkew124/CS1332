...
public class SortingTest {
    ...
    public void insertionStability() {
        ...
        Integer one1 = 1;
        Integer one2 = 1;
        Integer max0 = Integer.MAX_VALUE;
    ...
    public void quickSortInstability() {
        Integer zero0 = 0;
        Integer zero1 = 0;
        Integer[] arr = new Integer[] {1, zero0, zero1};

        BadRandom rand = new BadRandom();
        rand.load(Arrays.asList(2));
        Sorting.quickSort(arr, comp, rand);

        assertArrayEquals(new Integer[] {0, 0, 1}, arr);

        // these end up out in reversed order
        assertSame(arr[0], zero1);
        assertSame(arr[1], zero0);
    }
    ...
    @Test
    public void radixStability() {
        Integer one0 = 1;
        Integer one1 = 1;
        Integer one2 = 1;
        int[] arr = {8, 17, 200, 3, one0, 13, 215, one1, 8990, one2};

        Sorting.lsdRadixSort(arr);

        assertArrayEquals(new int[] {1, 1, 1, 3, 8, 13, 17, 200, 215, 8990}, arr);
        assertSame(one0, arr[0]);
        assertSame(one1, arr[1]);
        assertSame(one2, arr[2]);
    }
