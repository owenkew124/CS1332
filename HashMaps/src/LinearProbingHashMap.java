...
public class LinearProbingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        double s = size;
        if ((s + 1) / table.length >= MAX_LOAD_FACTOR) {
        ...
        if (old == null && removed) {
            table[removedInt] = new LinearProbingMapEntry<>(key, value);
            return old;
        } else {
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new IllegalArgumentException("The length given is not large enough"
                                         + "to contain the data in the existing HashMap");
        }
        LinearProbingMapEntry[] oldTable = table;
        this.table = new LinearProbingMapEntry[length];
        for (int i = 0; i < oldTable.length; i++) {
            if (oldTable[i] == null || oldTable[i].isRemoved()) {
                continue;
            }
            int code = oldTable[i].getKey().hashCode();
            int index = code % table.length;
